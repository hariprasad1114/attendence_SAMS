<?php
/**
 * SAMS API Test Script - Improved Version
 * This script tests all the API endpoints with better error handling
 */
...
// First, let's get a valid teacher ID
$teacherId = null;
$teacherResult = makeRequest($baseUrl . '/get_users.php?role=teacher');
if ($teacherResult['response'] && isset($teacherResult['response']['users'][0])) {
    $teacherId = $teacherResult['response']['users'][0]['id'];
} else {
    ...
    $teacherCreateResult = makeRequest($baseUrl . '/register_user.php', 'POST', $testTeacher);
    if ($teacherCreateResult['response'] && $teacherCreateResult['response']['success']) {
        $teacherId = $teacherCreateResult['response']['user']['id'];
    }
}
...
echo "🧪 SAMS API Test Suite - Improved Version\n";
echo "==========================================\n\n";
...
// Test 5: Generate Attendance Code (only if we have a valid teacher)
if ($teacherId) {
    testEndpoint(
    ...
    echo "Testing: Generate Attendance Code\n";
    echo "URL: $baseUrl/generate_code.php\n";
    echo "Method: POST\n";
    echo "⚠️  SKIPPED - No valid teacher ID available\n";
    echo str_repeat("-", 50) . "\n\n";
}
